...
pub(crate) fn generic_op(op: &Op, ty: &VecType) -> TokenStream {
    ...
    match op.sig {
        ...
        OpSig::Compress { merge: true } => {
            let len = Literal::usize_unsuffixed(ty.len);
            let truncate_mask = truncate_byte_mask(ty.len);
            quote! {
                #method_sig {
                    #truncate_mask
...
fn truncate_byte_mask(lanes: usize) -> TokenStream {
    if lanes == 64 {
        TokenStream::new()
    } else {
        let mask = Literal::u64_unsuffixed((1_u64 << lanes) - 1);
        quote! { let mask = mask & #mask; }
    }
}
